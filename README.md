...
3. **AI-агент (по умолчанию GPT через Codex CLI)** анализирует записи
...
|-----------|-------|-----------|
| **OpenAI (Codex) или Claude** | AI-агент, который обрабатывает ваши записи | по вашему тарифу |
...
**Итого: зависит от выбранного AI-провайдера + VPS**
...
Команды авторизации откроют ссылку/код — следуйте инструкциям на экране.
...
| Codex установлен | Команда `codex --version` показывает номер версии |
| Codex авторизован | Команда `codex login status` показывает `Logged in` |

---

## Запуск бота

По умолчанию бот использует `LLM_PROVIDER=openai-cli` (GPT через Codex CLI, без API-ключа).
...
| 🤖 GPT | Переключает активную модель на GPT (Codex CLI) |
| 🧠 Claude | Переключает активную модель на Claude CLI |
